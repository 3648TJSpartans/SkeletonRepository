...
public class DriveTo extends Command{
    ...
    private final ProfiledPIDController driveController = new ProfiledPIDController(
        goToConstants.drivekP, 0.0, goToConstants.drivekD, new TrapezoidProfile.Constraints(goToConstants.driveMaxVelocity,goToConstants.driveMaxAcceleration), 0.02);
    
    private final ProfiledPIDController thetaController = new ProfiledPIDController(
        goToConstants.thetakP, 0.0, goToConstants.thetakD, new TrapezoidProfile.Constraints(goToConstants.thetaMaxVelocity, goToConstants.thetaMaxAcceleration), 0.02);
    ...
    public void execute(){
        ...
        //Gets the displacement vector -- if the robot goes absolutely the wrong way, switch target pose and robot pose.
        Translation2d displacement = targetPose.getTranslation().minus(robotPose.getTranslation());
        //Defines translational speed the robot should go. displacement.getNorm is the magnitude of the displacement.
        double driveSpeed = driveController.calculate(displacement.getNorm());
        //sets velocity to the normalized displacement vector(direction of displacement) timesd the desired drive speed.
        Translation2d setVelocity = displacement.div(displacement.getNorm()).times(driveSpeed);

        //Gets the displacement angle -- if the robot rotates absolutely the wrong way, switch target pose and robot pose.
