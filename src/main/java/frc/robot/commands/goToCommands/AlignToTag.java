...
public class AlignToTag extends Command {
    ...
    public AlignToTag(Drive drive, Supplier<Pose2d> robotPose, Supplier<Pose2d> targetPose) {
    ...
    public AlignToTag(Drive drive, Supplier<Pose2d> targetPose) {
    ...
    public void execute() {
        ...
        // runs velocities
        drive.runVelocity(new ChassisSpeeds(setVelocity.getX(), setVelocity.getY(), thetaVelocity));

        // Lets Log stuff
        Logger.recordOutput("AlignToTag/displacement", displacement);
        Logger.recordOutput("AlignToTag/RobotPose", robotPose);
        Logger.recordOutput("AlignToTag/Trajectory", displacement);
        Logger.recordOutput("AlignToTag/TargetPose", targetPose);
        Logger.recordOutput("AlignToTag/setDriveSpeed", driveSpeed);
        Logger.recordOutput("AlignToTag/setDriveVelocity", setVelocity);
        Logger.recordOutput("AlignToTag/thetaDifference", thetaDisplacement);
        Logger.recordOutput("AlignToTag/thetaVelocity", thetaVelocity);
    }
